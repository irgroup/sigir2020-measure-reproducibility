...
def main():
    ...
    if settings.clean_dir:
        ...
        util.delete_shelve(file['feat_core17_robust04'])
    ...
    if settings.data_prep:
        ...
        if not settings.robust_only:
            if settings.preprocess:
                corpora = [path['core17_clean'], path['robust04_clean']]
                dprep.unify(path['union_core17_robust04'], corpora)
            else:
                corpora = [path['core17_raw'], path['robust04_raw']]
                dprep.unify(path['union_core17_robust04_raw'], corpora)

    # Generate tfidf-vectorizer
    if settings.tfidf_vec:
        if settings.robust_only:
            if settings.preprocess:
                tfidf_vec.dump_tfidf_vectorizer(file['vectorizer_core17_robust04'],
                                                path['robust04_clean'])
            else:
                tfidf_vec.dump_tfidf_vectorizer(file['vectorizer_core17_robust04'],
                                                path['robust04_raw'])
    ...
    if settings.prep_corpus_feature:
        if settings.preprocess:
            fprep.prepare_corpus_feature(file['vectorizer_core17_robust04'],
                                         path['core17_clean'],
                                         file['feat_core17_robust04'])
        else:
            fprep.prepare_corpus_feature(file['vectorizer_core17_robust04'],
                                         path['core17_raw'],
                                         file['feat_core17_robust04'])
    if settings.score:
        ...
        qrel_files = [file['qrel_core17'], file['qrel_robust04']]
        ...
        if topics is not None:
            ...
            if settings.robust_only:
                ...
                if settings.preprocess:
                    train_data = path['union_core17_robust04']
                else:
                    train_data = path['union_core17_robust04_raw']

            score.score(topics=topics,
                        vectorizer=file['vectorizer_core17_robust04'],
                        qrel=file['qrel_robust04'],
                        features=file['feat_core17_robust04'],
                        scores=file['score_core17_robust04'],
                        train_data=train_data,
                        train_features=path['train_feat'],
                        single_runs=path['single_runs'],
                        num_cpu=settings.num_cpu)

            complete_run_file = path['complete_run'] + 'replica_wcrobust04_' + str(settings.num_con)
